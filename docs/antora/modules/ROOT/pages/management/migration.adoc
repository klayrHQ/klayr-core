...
NOTE: Most likely you already have a `keys.json` file from the Lisk Core v4 node. Skip this step if you have already have a `keys.json` file.
...
[IMPORTANT]
====
You most likely already have a `keys.json` file from the Lisk Core v4 node. If you have already created the keys for the Klayr Core node, you need to update the address in the `keys.json` file to the new address of the validator.
By replacing the `lsk` prefix with `kly` in the address, you can use the same keys for the Klayr Core node.
====
