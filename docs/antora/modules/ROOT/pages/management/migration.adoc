...
:klayr-migrator: klayr-migrator-v2.0.1-linux-x64.tar.gz
:klayr-migrator_darwinx64: klayr-migrator-v2.0.1-darwin-x64.tar.gz
:klayr-migrator_darwinarm64: klayr-migrator-v2.0.1-darwin-arm64.tar.gz
:docs_sdk: klayr-sdk::
:docs_klayr: ROOT::
:docs_klayrv5: ROOT::
// External URLs
...
:url_klayr_migrator_darwinarm64: https://downloads.klayr.com/klayr-migrator/{klayr-migrator_darwinarm64}
:url_klayr_migrator_darwinx64: https://downloads.klayr.com/klayr-migrator/{klayr-migrator_darwinx64}
:url_wallet: https://klayr.com/wallet
:url_jsonrpc: https://www.jsonrpc.org/specification
:url_klayr_blog_migration: https://klayr.com/blog/posts/announcing-klayr-testnet-v4-migration
:url_klayr_blog_migration_mainnet: https://klayr.com/blog/posts/announcing-klayr-mainnet-v4-migration
// Project URLs
...
This guide explains how to migrate a Klayr Core v3 node to Klayr Core v4 using the Klayr Migrator v2.

The Klayr migrator CLI will generate a new genesis block for Klayr Core v4.
The new genesis block is created based on a snapshot of the old blockchain (running on Klayr Core v3.1.0 or later) at a given height.

[IMPORTANT]
====
All active validators on the Klayr Mainnet and the Klayr Testnet need to follow this guide to correctly migrate their nodes to the new network, in order to not miss any blocks after the network hard fork.

Optionally, anyone running a Klayr Core v3.1.0 (or later) who is not an active validator can also participate in the migration process.
...
=== Ensure you are running version 3.1.0 (or later) of Klayr Core
Ensure you are running version 3.1.0 (or later) of Klayr Core to be able to seamlessly migrate to Klayr Core 4.0.0.

We *recommend* to install *v3.1.1* of Klayr Core, because it comes with the Mainnet snapshot height pre-configured. If using an existing data directory, kindly make sure to use the `--overwrite-config` flag when restarting the application.

Print the Klayr Core node info in the terminal, and check the value of `version`. Also, kindly ensure that the `backup.height` property is present and the corresponding value matches the announced migration height of {snapshotHeight}.

[source,bash]
----
klayr-core node:info --pretty
----
...
=== Ensure Klayr Core v3.1.x is fully synced with the network
...
TIP: To view the current height of the *Klayr Testnet*, use the network switcher of Klayr Desktop, which can be enabled in the settings.

Alternatively, users can also verify the current height by comparing `data.height` in the response from the https://service.klayr.com/api/v2/network/status[^] endpoint.

To directly check the current height via the command line, run:

[source,bash]
----
curl --silent https://service.klayr.com/api/v2/network/status | jq '.data.height'
----
...
Open the necessary ports for Klayr Core v4.

[IMPORTANT]
====
If you migrate the existing Klayr Core v3 config with the `--auto-migrate-config` flag in the next step <<run-klayr-migrator>>, please ensure that the necessary ports in the final v4 config are open for communication.
...
[TIP]
====
In case a custom xref:{url_sdk_config_system}[dataPath] is defined in the config, that is different from the default path `~/.klayr/klayr-core`, then it is possible to define the data path with the `--klayr-core-v3-data-path` flag like so:

[source,bash,subs=attributes+]
----
klayr-migrator --snapshot-height {snapshotHeight} --klayr-core-v3-data-path ~/klayr/custom/path/  --auto-migrate-config --auto-start-klayr-core-v4
----
====
...
[TIP]
====
In case a xref:{url_build_custom_config}[custom config] is used for Klayr Core v3, then it is also possible to define the path to the custom config file with the `--config` flag like so:
...
[source,bash]
----
grep \"id\": ./klayr-migrator/output/4c09e6a781fc4c7bdb936ee815de8f94190f8a7519becd9de2081832be309a99/genesis_block.json
----
...
=== Stop Klayr Core v3.1.x

After the `klayr-migrator` script has finished and the announced snapshot height has passed, there is no reason to continue running Klayr Core v3.1.x, and therefore it is recommended to stop it.

[IMPORTANT]
====
It is important to stop Klayr Core v3 *before* starting Klayr Core v4.
If the migrator is started with `--auto-start-klayr-core-v4` flag, the user is prompted to verify that Klayr Core v3 is stopped - only afterwards, the migration script will continue.

.Prompt when
 Please stop Klayr Core v3 to continue. Type 'yes' and press Enter when ready. [yes/no]: yes
====
...
NOTE: Users that have set something up to start Klayr Core on boot, whether it's through `cron` or `systemd` or something else, should keep that in mind and adjust things accordingly so that Klayr Core 4.0.0 gets started instead of Klayr Core 3.1.x.

Last but not least, remove the folder with Klayr Core v3.1.x.


=== Start Klayr Core v4
If you set the flag `--auto-start-klayr-core-v4` when running klayr-migrator, it will start Klayr Core v4 in the background (managed by PM2) right after successful migration.
...
Check the value of `version` in the response, to verify you are running version 4.0.0 of Klayr Core.
...
The initial validators will be exactly the 101 validators that were in active positions in the Klayr Core v3 network, at the time of the migration.
...
Look in the output directory of the klayr-migrator for a file called `forgingStatus.json` (the specific location will be in the migrator output), this will contain the valid validator info data to enable block generation on the Klayr Core v4 node.

.Migrator output example
 Finished exporting forging status to /root/klayr-migrator/output/15f0dacc1060e91818224a94286b13aa04279c640bd5d6f193182031d133df7c/forgingStatus.json
...
[WARNING]
====
If the validator already generated blocks with Klayr Core v3, they need to use their current validator info data.
...
The migration of Klayr Core from v3 to v4 is now completed.
